...
export default function ImageGrid({ filtered, currentFilter }: ImageGridProps) {
  ...
  function renderCard(img: ImageItem) {
    ...
    return (
      <div key={`${currentFilter}-${img.id}`} className="h-full border border-zinc-800 rounded-xl bg-zinc-900/20 p-5 flex flex-col hover:border-zinc-700 transition-colors" onMouseEnter={() => void warmReview(img.id)} onFocusCapture={() => void warmReview(img.id)}>
        ...
        <Button href={`/expert/annotate/${img.id}`} variant={needsCurrentReview ? 'primary' : 'secondary'} className="mt-auto w-full justify-center">{needsCurrentReview ? 'Verify Classification' : canAudit ? 'Review again' : img.review.hasReviewed ? 'Update my review' : 'View Details'}</Button>
      </div>
    )
  }
