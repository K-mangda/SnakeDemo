...
import { CheckCircle2, Clock, HelpCircle, PlusCircle, SearchX } from 'lucide-react'
...
export default function StatusBadge({ status, label }: StatusBadgeProps) {
  ...
  switch (status) {
    ...
    case 'no_detection':          return <Badge className={baseClass} variant="muted"><SearchX size={12} className="mr-1 shrink-0" />{label ?? 'No detection'}</Badge>
