    ...
    <AuthShell label="" title="" description="">
      ...
      {invitationInvalid ? (
        ...
        <>
          ...
          {ready && !completed ? <form onSubmit={submit} className="space-y-5"><label className="block text-xs font-medium text-zinc-400">New password<div className="relative mt-2"><input type={showPassword ? 'text' : 'password'} value={password} onChange={event => setPassword(event.target.value)} placeholder="Create a new password" className="w-full rounded-xl border border-white/[0.09] bg-white/[0.04] px-4 py-3 pr-12 text-sm text-zinc-100 outline-none transition placeholder:text-zinc-600 focus:border-emerald-400/70 focus:bg-emerald-400/[0.04]" /><button type="button" tabIndex={-1} onClick={() => setShowPassword(!showPassword)} aria-label={showPassword ? 'Hide password' : 'Show password'} className="absolute inset-y-0 right-0 grid w-12 place-items-center text-zinc-500 hover:text-zinc-300">{showPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button></div><span className={`mt-1.5 block text-xs font-normal transition-colors ${charactersRemaining === 0 ? 'text-emerald-400' : 'text-zinc-500'}`}>{charactersRemaining === 0 ? 'Password length looks good' : `${charactersRemaining} character${charactersRemaining === 1 ? '' : 's'} remaining`}</span></label><label className="block text-xs font-medium text-zinc-400">Confirm password<div className="relative mt-2"><input type={showConfirmation ? 'text' : 'password'} value={confirmPassword} onChange={event => setConfirmPassword(event.target.value)} placeholder="Repeat your new password" className="w-full rounded-xl border border-white/[0.09] bg-white/[0.04] px-4 py-3 pr-12 text-sm text-zinc-100 outline-none transition placeholder:text-zinc-600 focus:border-emerald-400/70 focus:bg-emerald-400/[0.04]" /><button type="button" tabIndex={-1} onClick={() => setShowConfirmation(!showConfirmation)} aria-label={showConfirmation ? 'Hide password' : 'Show password'} className="absolute inset-y-0 right-0 grid w-12 place-items-center text-zinc-500 hover:text-zinc-300">{showConfirmation ? <EyeOff size={17} /> : <Eye size={17} />}</button></div></label><Button disabled={saving} className="w-full !rounded-xl !py-3.5" size="lg">{saving ? 'Saving…' : preview ? 'Preview only' : 'Set password'}</Button></form> : !completed ? <p className="text-sm text-zinc-400">{message}</p> : null}
