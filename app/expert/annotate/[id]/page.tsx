          ...
          <section className="overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900/20">
            ...
            <div className="flex min-h-[420px] items-center justify-center bg-zinc-950 p-4 sm:p-6">
              <div ref={imageRef} className={`relative inline-block max-h-[600px] max-w-full select-none ${placingBox ? 'cursor-crosshair' : ''}`} onMouseDown={beginDrawingBox} onMouseMove={updateBox} onMouseUp={endBoxAction} onMouseLeave={endBoxAction}>
                ...
                {bbox && <div className="absolute cursor-move border-2 border-emerald-400 bg-emerald-500/10 shadow-[0_0_18px_rgba(16,185,129,0.22)]" style={{ left: `${bbox.x}%`, top: `${bbox.y}%`, width: `${bbox.width}%`, height: `${bbox.height}%` }} onMouseDown={(event) => beginBoxAction(event, 'move')}><span className={`absolute -top-7 left-0 max-w-[220px] truncate rounded px-2 py-1 text-[10px] font-medium ${hasExpertSelectedSpecies ? 'bg-emerald-500 text-zinc-950' : 'bg-amber-400 text-zinc-950'}`}>{boxLabel}</span><ResizeHandles onStart={beginBoxAction} /></div>}
              </div>
            </div>
