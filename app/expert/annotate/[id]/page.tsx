      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid gap-7 lg:grid-cols-[minmax(0,1.45fr)_minmax(340px,0.8fr)]">
          ...
          <aside className="space-y-5">
            ...
            {scan.reviewHistory.length > 0 && <section className="rounded-2xl border border-zinc-800 bg-zinc-900/10 p-5"><div className="flex items-baseline justify-between gap-3"><p className="text-xs uppercase tracking-widest text-zinc-400">Review history</p><span className="text-[11px] text-zinc-600">Visible after your review</span></div><div className="mt-3 space-y-2">{scan.reviewHistory.map((review, index) => <div key={`${review.reviewer}-${review.createdAt}-${index}`} className="flex items-center justify-between gap-3 rounded-lg border border-zinc-800/80 bg-zinc-950/50 px-3 py-2.5"><span className="truncate text-xs text-zinc-400">{review.reviewer}</span><span className={`truncate text-right text-xs font-medium ${review.species ? 'italic text-zinc-200' : 'text-amber-400'}`}>{review.species ?? 'Unclear'}</span></div>)}</div></section>}
