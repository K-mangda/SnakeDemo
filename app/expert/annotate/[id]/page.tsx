...
type Scan = {
  id: string; originalFilename: string; createdAt: string; imageUrl: string; confidence: number | null; bbox: Box | null
...
export default function AnnotatePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="min-h-screen pt-28 px-6 pb-20"><div className="max-w-5xl mx-auto">
    <header className="mb-8 border-b border-zinc-900 pb-6"><Button variant="ghost" size="sm" onClick={() => router.push('/expert')} className="mb-4 -ml-2 text-zinc-500"><ArrowLeft size={16} /> Back to Workspace</Button><h1 className="text-2xl font-medium text-zinc-100">Verification task</h1><p className="mt-1 text-sm font-mono text-zinc-500" title={`Original file: ${scan.originalFilename}`}>{formatScanLabel(scan.createdAt)}</p></header>
