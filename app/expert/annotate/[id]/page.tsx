        ...
        <div className="grid gap-7 lg:grid-cols-[minmax(0,1.45fr)_minmax(340px,0.8fr)]">
          ...
          <aside className="space-y-5">
            ...
            <section className="rounded-2xl border border-zinc-800 bg-zinc-900/30 p-5"><p className="text-xs uppercase tracking-widest text-zinc-500">Your decision</p><div className="mt-4 space-y-2"><DecisionOption active={decision === 'pending'} onClick={() => setDecision('pending')} title="Confirm or correct species" detail="Select the species you believe is in the image." /><DecisionOption active={decision === 'unclear'} onClick={() => setDecision('unclear')} title="Cannot identify confidently" detail="Counts as your review without selecting a species." tone="amber" /><DecisionOption active={decision === 'waiting_for_new_class'} onClick={() => setDecision('waiting_for_new_class')} title="Species is not in the list" detail="Flag this image for a new reference class." tone="blue" /></div>
              {decision === 'pending' && <div className="mt-5 border-t border-zinc-800 pt-5"><label className="text-xs font-medium text-zinc-400">Reference species</label><div className="relative mt-2"><Search size={15} className="pointer-events-none absolute left-3 top-3 text-zinc-500" /><input value={search} onFocus={() => setSpeciesMenuOpen(true)} onChange={(event) => { setSearch(event.target.value); setSpeciesMenuOpen(true) }} placeholder="Search scientific or Thai name…" className="w-full rounded-lg border border-zinc-800 bg-zinc-950 py-2.5 pl-9 pr-3 text-sm text-zinc-200 outline-none transition-colors placeholder:text-zinc-600 focus:border-emerald-500" />{speciesMenuOpen && <div className="absolute z-20 mt-2 max-h-64 w-full overflow-y-auto rounded-xl border border-zinc-700 bg-zinc-950 p-1.5 shadow-2xl shadow-black/50">{matchingSpecies.length ? matchingSpecies.slice(0, 8).map((item) => { const isSelected = item.id === selectedSpeciesId; return <button key={item.id} type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => { setSelectedSpeciesId(item.id); setSearch(''); setSpeciesMenuOpen(false) }} className={`flex w-full items-center justify-between gap-3 rounded-lg px-3 py-2.5 text-left transition-colors ${isSelected ? 'bg-emerald-500/15 text-emerald-300' : 'text-zinc-200 hover:bg-zinc-800'}`}><span className="min-w-0"><span className="block truncate text-sm font-medium italic">{item.scientific_name}</span><span className="mt-0.5 block truncate text-xs text-zinc-500">{item.name_th ?? item.name_en ?? 'No common name'}</span></span>{isSelected && <Check size={16} className="shrink-0" />}</button> }) : <p className="px-3 py-4 text-center text-xs text-zinc-500">No reviewed species found.</p>} {matchingSpecies.length > 8 && <p className="border-t border-zinc-800 px-3 pt-2 text-[11px] text-zinc-500">Type more to narrow {matchingSpecies.length} results.</p>}</div>}</div>{selectedSpecies && <div className="mt-3 flex items-center gap-2 rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-xs text-emerald-300"><Check size={14} className="shrink-0" /><span className="truncate">Selected: <span className="italic">{selectedSpecies.scientific_name}</span>{selectedSpecies.name_th ? ` — ${selectedSpecies.name_th}` : ''}</span></div>}</div>}
